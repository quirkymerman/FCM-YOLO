...
from model.MSHNet_NSFPN import *
...
def parse_args():
    ...
    parser.add_argument('--dataset-dir', type=str, default='/path/to/Datasets/')
...
class Trainer(object):
    def __init__(self, args):
        ...
        self.train_loader = Data.DataLoader(trainset, args.batch_size, shuffle=True, drop_last=True, num_workers=32)
        self.val_loader = Data.DataLoader(valset, 1, drop_last=False, num_workers=32)
        ...
        if args.mode=='train':
            if args.if_checkpoint: # For Resume
                check_folder = '/path/to/weight_folder/'
                ...
                self.save_folder = './weight/IRSTD-1k_MSHNet_NSFPN-%s' % (time.strftime('%Y-%m-%d-%H-%M-%S', time.localtime(time.time())))
