...
class MSHNet_NSFPN(nn.Module):
    def __init__(self, input_channels, block=ResNet):
        ...
        self.decoder_3 = self._make_layer(fpn_outchannels * 2, param_channels[3], block,
                                          param_blocks[2])
        self.decoder_2 = self._make_layer(fpn_outchannels + param_channels[3], param_channels[2], block,
                                          param_blocks[1])
        self.decoder_1 = self._make_layer(fpn_outchannels + param_channels[2], param_channels[1], block,
                                          param_blocks[0])
    ...
    def forward(self, x, warm_flag):
        x_e0 = self.encoder_0(self.conv_init(x)) # 16, /1
        x_e1 = self.encoder_1(self.pool(x_e0))   # 32, /2
        x_e2 = self.encoder_2(self.pool(x_e1))   # 64, /4
        x_e3 = self.encoder_3(self.pool(x_e2))   # 128, /8
        x_m = self.middle_layer(self.pool(x_e3)) # 256, /16
